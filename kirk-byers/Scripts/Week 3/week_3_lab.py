...
for line in show_arp.splitlines():
    ...
    if ip_addr == "10.220.88.1":
        print("Default GW IP/Mac is: {}/{}".format(ip_addr, mac_addr))
        found_1 = True
...
base_ip = "10.10.100."
linux_cmd = "ping -c 2"
windows_cmd = "ping -n 2"

cmd = windows_cmd if WINDOWS else linux_cmd
...
for i, ip_addr in enumerate(list):
...
for ip_addr in list:
    ...
    return_code = os.system("{} {}".format(cmd, ip_addr))
    print('-' * 80)
